...
def searchsuggestions(request):
    ...
    medicine_name = []
    results = Medicine1mg.objects.filter(name__startswith = medicine_prefix).values('name')
    results += MedicineNetMeds.objects.filter(name__startswith = medicine_prefix).values('name')
    results += MedicinePharmEasy.objects.filter(name__startswith = medicine_prefix).values('name')
